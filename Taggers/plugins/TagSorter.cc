        ...
        if ( SelectedTag->size() == 1  && storeOtherTagInfo_ && blindedSelectionPrintout_ ) {
            ...
            if (mass < 115. || mass > 135.) {
                ...
                if( SelectedTagTruth->size() != 0 ) {
                    std::cout << "* HTXS Category 0 (1) {1.1} [1.1 fine]: " << SelectedTagTruth->back().HTXSstage0cat() << " ("
                              << SelectedTagTruth->back().HTXSstage1cat() << ")" << " {"
                              << SelectedTagTruth->back().HTXSstage1_1_cat() << "}" << " ["
                              << SelectedTagTruth->back().HTXSstage1_1_fine_cat() << "]" << std::endl;
